...
public class ThrowPot extends Module {
    ...
    private boolean lambda$onKeyBindingIsPressed$0(ItemStack var1) {
        if (!(var1.getItem() instanceof SplashPotionItem)) {
            ...
            Iterator var5 = var1.get(DataComponentTypes.POTION_CONTENTS).getEffects().iterator();
            if (var5 != null && var5.hasNext()) {
                while (var5.hasNext()) {
                    StatusEffectInstance var6 = (StatusEffectInstance) var5.next();
                    if (var6.getEffectType().value().isBeneficial() && this.field2830.getValue() == ThrowPotMode.DeBuff) {
                        return false;
                    }

                    if (!var6.getEffectType().value().isBeneficial() && this.field2830.getValue() == ThrowPotMode.Buff) {
                        return false;
                    }
                }
