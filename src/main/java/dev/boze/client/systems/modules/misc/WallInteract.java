...
public class WallInteract extends Module {
   ...
   private void method1780(PrePlayerTickEvent var1) {
      if (!mc.player.isSneaking() && mc.options.useKey.isPressed()) {
         Iterable var5 = Class5914.method19();
         ...
         Vec3d var16 = new Vec3d(0.0, 0.0, 0.1)
            .rotateX(-((float)Math.toRadians((double)mc.player.getPitch())))
            .rotateY(-((float)Math.toRadians((double)mc.player.getYaw())));
         this.field3136.clear();

         for (int var17 = 1; (double)var17 < Reach.method1614() * 10.0; var17++) {
            BlockPos var8 = BlockPos.ofFloored(mc.player.getCameraPosVec(mc.getRenderTickCounter().getTickDelta(true)).add(var16.multiply((double)var17)));
