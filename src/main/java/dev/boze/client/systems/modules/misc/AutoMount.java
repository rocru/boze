...
public class AutoMount extends Module {
    ...
    public void method1670(MovementEvent event) {
        if (!mc.player.isRiding() && mc.player.getVehicle() == null && !(mc.currentScreen instanceof HorseScreen)) {
            if (this.timer.hasElapsed(this.delay.getValue() * 1000.0F)) {
                this.timer.reset();
                ...
                if (var8 != null) {
                    ...
                    event.method1074(new ActionWrapper(() -> lambda$onUpdateWalkingPlayer$1(var8), var9[0], var9[1]));
                }
            }
        } else {
            this.timer.reset();
    ...
    private boolean method1671(Entity var1) {
        if (var1.distanceTo(mc.player) > this.range.getValue()) {
            ...
            return true;
        } else if (var1 instanceof PigEntity var5 && this.pigs.getValue()) {
