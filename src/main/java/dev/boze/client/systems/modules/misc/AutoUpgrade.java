...
public class AutoUpgrade extends Module {
   ...
   private boolean method1688() {
      if (mc.currentScreen instanceof SmithingScreen var4) {
         if (((SmithingScreenHandler)var4.getScreenHandler()).getSlot(0).getStack().getItem() != Items.NETHERITE_UPGRADE_SMITHING_TEMPLATE) {
            int var8 = InventoryHelper.method169(AutoUpgrade::lambda$handle$1);
            if (var8 == -1) {
               return false;
            } else {
               mc.interactionManager
                  .clickSlot(((SmithingScreenHandler)var4.getScreenHandler()).syncId, SlotUtils.method1541(var8), 0, SlotActionType.QUICK_MOVE, mc.player);
               return true;
            }
         } else if (((SmithingScreenHandler)var4.getScreenHandler()).getSlot(2).getStack().getItem() != Items.NETHERITE_INGOT) {
            int var7 = InventoryHelper.method169(AutoUpgrade::lambda$handle$2);
            if (var7 == -1) {
               return false;
            } else {
               mc.interactionManager
                  .clickSlot(((SmithingScreenHandler)var4.getScreenHandler()).syncId, SlotUtils.method1541(var7), 0, SlotActionType.QUICK_MOVE, mc.player);
               return true;
            }
         } else if (!((SmithingScreenHandler)var4.getScreenHandler()).getSlot(3).getStack().isEmpty()) {
            mc.interactionManager.clickSlot(((SmithingScreenHandler)var4.getScreenHandler()).syncId, 3, 0, SlotActionType.QUICK_MOVE, mc.player);
            return true;
         } else {
            ...
            if (var6 == -1) {
               ...
               mc.interactionManager
                  .clickSlot(((SmithingScreenHandler)var4.getScreenHandler()).syncId, SlotUtils.method1541(var6), 0, SlotActionType.QUICK_MOVE, mc.player);
               return true;
            }
         }
      } else {
   ...
   private static boolean lambda$handle$3(ItemStack var0) {
      ...
      if (var4 instanceof ToolItem var5) {
         ...
         return var4 instanceof ArmorItem var6 ? var6.getMaterial() == ArmorMaterials.DIAMOND : false;
      }
   }
