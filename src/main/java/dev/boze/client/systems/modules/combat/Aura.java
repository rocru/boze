...
public class Aura extends Module {
    ...
    private void method1404(Render3DEvent var1) {
        if (this.field2459.isGhostMode()) {
            ...
            if (this.render.getValue() && this.al != null && !this.am.hasElapsed(2500.0)) {
                double var5 = MathHelper.lerp(var1.field1951, this.al.lastRenderX, this.al.getX()) - this.al.getX();
                double var7 = MathHelper.lerp(var1.field1951, this.al.lastRenderY, this.al.getY()) - this.al.getY();
                double var9 = MathHelper.lerp(var1.field1951, this.al.lastRenderZ, this.al.getZ()) - this.al.getZ();
    ...
    private boolean method1409() {
        if (!this.ai.hasElapsed(this.swapDelay.getValue() * 50.0F)) {
            return false;
        } else if (!this.method1413(mc.player.getMainHandStack().getItem())) {
            return false;
        } else if (this.delay.getValue() == DelayMode.Tick && !this.aj.hasElapsed((float) this.ticks.getValue().intValue() * 50.0F)) {
            return false;
        } else {
