...
public class PathBuilder implements IMinecraft {
    public static Path method616(ArrayList<PathPos> path, double maxAcceleration, double maxSpeed) {
        Path var8 = new Path(method617(path.get(0)));
        Vec3d var9 = method617(path.get(0)).subtract(mc.player.getPos());
        if (var9.length() > 0.01) {
        ...
        if (path.size() < 2) {
            ...
            Vec3d var10 = method617(path.get(1)).subtract(method617(path.get(0)));

            for (int var11 = 1; var11 < path.size() - 1; var11++) {
                Vec3d var12 = method617(path.get(var11));
                Vec3d var13 = method617(path.get(var11 + 1));
    ...
    private static Vec3d method617(PathPos var0) {
        return new Vec3d((double) var0.getX() + 0.5, var0.getY(), (double) var0.getZ() + 0.5);
    }
