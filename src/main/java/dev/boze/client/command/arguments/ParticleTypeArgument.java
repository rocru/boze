...
class ParticleTypeArgument implements ArgumentType<ParticleType<?>> {
   ...
   public ParticleType<?> method1007(StringReader reader) {
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
      return CommandSource.suggestMatching(
         (Iterable)Registries.PARTICLE_TYPE.stream().map(ParticleTypeArgument::lambda$listSuggestions$0).collect(Collectors.toList()), builder
      );
   }

   public Collection<String> getExamples() {
      return Arrays.asList(
         Registries.PARTICLE_TYPE.getId((ParticleType)Registries.PARTICLE_TYPE.get(0)).getPath(),
         Registries.PARTICLE_TYPE.getId((ParticleType)Registries.PARTICLE_TYPE.get(1)).getPath(),
         Registries.PARTICLE_TYPE.getId((ParticleType)Registries.PARTICLE_TYPE.get(2)).getPath()
      );
   }
