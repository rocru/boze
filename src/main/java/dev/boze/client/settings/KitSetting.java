...
public class KitSetting extends Setting<String> implements IMinecraft {
    ...
    private int lambda$build$1(CommandContext var1) throws CommandSyntaxException {
        File var5 = new File(ConfigManager.kits, var1.getArgument("title", String.class) + ".json");
        if (var5.exists()) {
            String var6 = this.setValue((String) var1.getArgument("title", String.class));
            if (var6.equals(var1.getArgument("title", String.class))) {
                ChatInstance.method624("Loaded kit " + var1.getArgument("title", String.class));
            } else {
                ChatInstance.method626("Error loading kit " + var1.getArgument("title", String.class));
            }
        } else {
