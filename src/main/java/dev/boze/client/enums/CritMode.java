...
public enum CritMode implements IMinecraft {
   ...
   private CritMode(BooleanSupplier var3) {
      this.field58 = var3;
   }

   private static boolean lambda$static$2() {
      return !MinecraftUtils.isClientActive() ? false : mc.player.isOnGround() || (double)mc.player.fallDistance > 0.01;
   }
   ...
   private static boolean lambda$static$1() {
      if (!MinecraftUtils.isClientActive()) {
         ...
         return mc.player.isOnGround() ? false : (double)mc.player.fallDistance > 0.01;
      }
   }
