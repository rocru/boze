...
public class MacroManagerSettingComponent extends ScaledSettingBaseComponent implements IMinecraft {
    private final MacroManagerSetting field301;

    public MacroManagerSettingComponent(MacroManagerSetting setting, BaseComponent parent, double x, double y, double width, double height) {
        super(setting, parent, x, y, width, height);
        this.field301 = setting;
    }

    @Override
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        if (this.field301.method2116()) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.field301.method2116() && isMouseWithinBounds(mouseX, mouseY, this.field318, this.field319, this.field320, this.field321)) {
            if (button == 0) {
                mc.getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_BUTTON_CLICK, 1.0F));
                ClickGUI.field1335.method580(new MacroManagerComponent(this.field301));
                return true;
            }

            if (button == 1 && this.field301.hasChildren()) {
                this.field301.setExpanded(!this.field301.isExpanded());
                return true;
            }
        }
