...
import dev.boze.client.gui.components.scaled.bottomrow.ProfileSettingScreenComponent;
...
public class ProfileSettingComponent extends ScaledSettingBaseComponent implements IMinecraft {
    private final ProfileSetting field306;

    public ProfileSettingComponent(ProfileSetting setting, BaseComponent parent, double x, double y, double width, double height) {
        super(setting, parent, x, y, width, height);
        this.field306 = setting;
    }

    @Override
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        if (this.field306.method2116()) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.field306.method2116() && isMouseWithinBounds(mouseX, mouseY, this.field318, this.field319, this.field320, this.field321)) {
            if (button == 0) {
                mc.getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_BUTTON_CLICK, 1.0F));
                ClickGUI.field1335.method580(new ProfileSettingScreenComponent(this.field306));
                return true;
            }

            if (button == 1 && this.field306.hasChildren()) {
                this.field306.setExpanded(!this.field306.isExpanded());
                return true;
            }
        }
