...
public abstract class ScaledBaseComponent implements IMinecraft {
   ...
   public ScaledBaseComponent(String name, double width, double height) {
      this.field1387 = name;
      this.field1390 = Math.min(width * 1280.0 * BaseComponent.scaleFactor, (double)mc.getWindow().getScaledWidth());
      this.field1391 = Math.min(height * 720.0 * BaseComponent.scaleFactor, (double)mc.getWindow().getScaledHeight());
