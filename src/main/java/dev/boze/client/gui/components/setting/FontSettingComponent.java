...
public class FontSettingComponent extends ScaledSettingBaseComponent implements IMinecraft {
    private final FontSetting field286;

    public FontSettingComponent(FontSetting setting, BaseComponent parent, double x, double y, double width, double height) {
        super(setting, parent, x, y, width, height);
        this.field286 = setting;
    }

    @Override
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        if (this.field286.method2116()) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.field286.method2116() && isMouseWithinBounds(mouseX, mouseY, this.field318, this.field319, this.field320, this.field321)) {
            if (button == 0) {
                mc.getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_BUTTON_CLICK, 1.0F));
                ClickGUI.field1335.method580(new FontSelectComponent(this.field286));
                return true;
            }

            if (button == 1 && this.field286.hasChildren()) {
                this.field286.setExpanded(!this.field286.isExpanded());
                return true;
            }
        }
