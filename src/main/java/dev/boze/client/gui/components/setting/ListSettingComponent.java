...
import dev.boze.client.gui.components.scaled.bottomrow.SelectFileComponent;
...
public class ListSettingComponent extends ScaledSettingBaseComponent implements IMinecraft {
    private final ListSetting field401;

    public ListSettingComponent(ListSetting setting, BaseComponent parent, double x, double y, double width, double height) {
        super(setting, parent, x, y, width, height);
        this.field401 = setting;
    }

    @Override
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        if (this.field401.method2116()) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.field401.method2116() && isMouseWithinBounds(mouseX, mouseY, this.field318, this.field319, this.field320, this.field321)) {
            if (button == 0) {
                mc.getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_BUTTON_CLICK, 1.0F));
                ClickGUI.field1335.method580(new SelectFileComponent(this.field401));
                return true;
            }

            if (button == 1 && this.field401.hasChildren()) {
                this.field401.setExpanded(!this.field401.isExpanded());
                return true;
            }
        }
