...
public class EyedropperScreen extends Screen implements IMinecraft {
   ...
   public void init() {
      ...
      this.field1053 = (Integer)mc.options.getGuiScale().getValue();
      mc.options.getGuiScale().setValue(1);
      mc.getWindow().setScaleFactor((double)mc.getWindow().calculateScaleFactor(1, mc.forcesUnicodeFont()));
   }

   public void close() {
      mc.options.hudHidden = this.field1052;
      mc.options.getGuiScale().setValue(this.field1053);
      mc.getWindow().setScaleFactor((double)mc.getWindow().calculateScaleFactor(this.field1053, mc.forcesUnicodeFont()));
      super.close();
   }
