...
public class RotationHelper implements IMinecraft {
   ...
   public static void updateRotation() {
      ...
      field3956 = new Vec3d((double)var3.x, (double)(-var3.y), (double)var3.z)
         .rotateX(-((float)Math.toRadians((double)mc.gameRenderer.getCamera().getPitch())))
         .rotateY(-((float)Math.toRadians((double)mc.gameRenderer.getCamera().getYaw())))
   ...
   private static void applyRotationToMatrix(MatrixStack var0) {
      if (mc.getCameraEntity() instanceof PlayerEntity var5) {
         ...
         var0.translate(
            -((double)(MathHelper.sin(var8 * (float) Math.PI) * var9) * 0.5), (double)(-(-Math.abs(MathHelper.cos(var8 * (float) Math.PI) * var9))), 0.0
         );
