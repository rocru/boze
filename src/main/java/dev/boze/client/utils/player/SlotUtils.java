...
public class SlotUtils implements IMinecraft {
    ...
    public static List<Slot> method664(ScreenHandler screenHandler) {
        return screenHandler.slots.stream().filter(Objects::nonNull).filter(Slot::isEnabled).collect(Collectors.toList());
    }
    ...
    public static Slot method665(ScreenHandler screenHandler, HandledScreenAccessor original, Vec2f lastMouseClick, BiPredicate<Slot, List<Slot>> block) {
        List<Slot> list = SlotUtils.method664(screenHandler);
        return list.stream().filter(arg_0 -> SlotUtils.lambda$getClosestSlot$0(block, list, arg_0)).min(Comparator.comparingDouble(arg_0 -> SlotUtils.lambda$getClosestSlot$1(lastMouseClick, original, arg_0))).orElse(null);
    }

    private static double lambda$getClosestSlot$1(Vec2f var0, HandledScreenAccessor var1, Slot var2) {
        return var0.distanceSquared(method663(var1, var2));
    }
