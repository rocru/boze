...
public class StackDeserializer implements IMinecraft {
   public static boolean method1756(ItemStack itemStack) {
      ContainerComponentAccessor var4 = (ContainerComponentAccessor)itemStack.get(DataComponentTypes.CONTAINER);
      if (var4 != null && !var4.getStacks().isEmpty()) {
         return true;
      } else {
         NbtCompound var5 = ((NbtComponent)itemStack.getOrDefault(DataComponentTypes.BLOCK_ENTITY_DATA, NbtComponent.DEFAULT)).getNbt();
   ...
   public static void method670(ItemStack itemStack, ItemStack[] items) {
      if (itemStack.getItem() == Items.ENDER_CHEST) {
         for (int var10 = 0; var10 < InventoryTracker.field1360.size(); var10++) {
            items[var10] = (ItemStack)InventoryTracker.field1360.get(var10);
         }
      } else {
         Arrays.fill(items, ItemStack.EMPTY);
         ComponentMap var5 = itemStack.getComponents();
         if (var5.contains(DataComponentTypes.CONTAINER)) {
            ContainerComponentAccessor var6 = (ContainerComponentAccessor)var5.get(DataComponentTypes.CONTAINER);
            ...
            NbtComponent var11 = (NbtComponent)var5.get(DataComponentTypes.BLOCK_ENTITY_DATA);
