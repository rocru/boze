...
public class RotationHelper implements IMinecraft {
    ...
    private final float field1363;
    private final float field1364;
    ...
    public RotationHelper method601(RotationHelper prevRotation, Function<RotationHelper, Boolean> preference) {
        ...
        Pair<Double, Double>[] var9 = method614(var8);
        RotationHelper r5 = prevRotation;
        List<RotationHelper> var11 = Arrays.stream(var9).map(arg_0 -> RotationHelper.lambda$correctSensitivity$0(r5, arg_0)).collect(Collectors.toList());
        if (preference != null) {
            Optional<RotationHelper> var12 = var11.stream().filter(preference::apply).findFirst();
            if (var12.isPresent()) {
                return var12.get();
            }
        }

        return var11.stream().min(this::lambda$correctSensitivity$1).orElse(null);
    }

    public RotationHelper method602(RotationHelper target, Pair<Double, Double> aimSpeed) {
        if (aimSpeed.getLeft() > aimSpeed.getRight()) {
            aimSpeed = new Pair(aimSpeed.getRight(), aimSpeed.getLeft());
        }

        double var6;
        if (aimSpeed.getLeft() == 1.0 && aimSpeed.getRight() == 1.0) {
            var6 = 1.0;
        } else {
            double var8;
            if (aimSpeed.getLeft().equals(aimSpeed.getRight())) {
                var8 = aimSpeed.getLeft();
            } else {
                var8 = ThreadLocalRandom.current().nextDouble(aimSpeed.getLeft(), aimSpeed.getRight());
            }
    ...
    public Vec3d method1954() {
        double var1 = Math.toRadians(this.method1384());
        double var3 = Math.toRadians(this.method1385());
    ...
    public int hashCode() {
        return Objects.hash(this.field1363, this.field1364);
    }
